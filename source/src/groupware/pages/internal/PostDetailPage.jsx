...
<div className="gw-admin-actions">{data.post.can_edit && <Link className="gw-secondary-button" to={`/boards/${boardSlug}/posts/${postId}/edit`}>수정</Link>}{data.post.can_delete && <button className="gw-secondary-button gw-secondary-button--danger" type="button" onClick={async () => { if (window.confirm('이 게시글을 삭제하시겠습니까?')) { await deleteBoardPost(postId); navigate(`/boards/${boardSlug}`); } }}>삭제</button>}</div></header><section className="gw-post-content"><BoardDocumentRenderer documentValue={documentValue} attachments={data.attachments} /></section>
    ...
    <div className="gw-admin-actions" style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'flex-end' }}><Link className="gw-secondary-button" to={`/boards/${boardSlug}`}>목록 보기</Link></div>
  </article>;
}
