...
const SETTINGS = { show_in_sidebar: true, allow_comments: true, allow_replies: true, allow_attachments: true, allow_images: true, allow_anonymous: false, allow_notices: true, allow_important: true, show_views: true, show_author_department: true, show_author_position: false, show_author_job_title: true, allow_reactions: true, show_post_number: true, search_enabled: true, use_prefix: false, use_pinned: true, department_only: false, page_size: 20, default_sort: 'latest', max_file_size_mb: 20, max_inline_image_size_mb: 10, max_inline_images: 20, max_total_attachment_mb: 50, preserve_image_originals: false, shortcut_enabled: false, shortcut_label: '', shortcut_url: '' };
  ...
  return <section className="gw-admin-section" aria-labelledby="board-builder-title">
    ...
    <div className="gw-builder-layout"><aside className="gw-builder-sidebar"><div className="gw-builder-sidebar-heading"><strong>게시판 목록</strong><span>수정할 게시판을 선택하세요.</span></div><div className="gw-compact-list" aria-label="게시판 목록">{shownBoards.map((board) => <button type="button" className={form.id === board.id ? 'is-selected' : ''} key={board.id} onClick={() => selectBoard(board)}><strong>{board.name}</strong><span>{getBoardType(board.board_type).label} · /{board.slug}{board.archived_at ? ' · 보관' : ''}</span></button>)}{shownBoards.length === 0 && <p>{showArchived ? '등록된 게시판이 없습니다.' : '운영 중인 게시판이 없습니다.'}</p>}{archivedCount > 0 && <button type="button" className="gw-archived-toggle" onClick={() => setShowArchived((current) => !current)}>{showArchived ? '보관함 숨기기' : `보관함 보기 (${archivedCount}개)`}</button>}</div></aside>
      <form className="gw-builder-form" onSubmit={submit}>
        ...
        <fieldset className="gw-builder-fieldset"><legend>5. 상단 바로가기 버튼</legend><p>게시판 화면 상단에 버튼을 하나 추가해 외부 링크나 다른 페이지로 바로 이동시킬 수 있습니다.</p><label><input type="checkbox" checked={Boolean(form.settings.shortcut_enabled)} onChange={(event) => updateSetting('shortcut_enabled', event.target.checked)} /> 바로가기 버튼 사용</label>{form.settings.shortcut_enabled && <div className="gw-admin-form-grid"><label className="gw-field"><span>버튼 제목</span><input maxLength="40" placeholder="예: 지입 계약서 양식" value={form.settings.shortcut_label ?? ''} onChange={(event) => updateSetting('shortcut_label', event.target.value)} /></label><label className="gw-field"><span>링크(URL)</span><input type="url" placeholder="https://..." value={form.settings.shortcut_url ?? ''} onChange={(event) => updateSetting('shortcut_url', event.target.value)} /></label></div>}</fieldset>
