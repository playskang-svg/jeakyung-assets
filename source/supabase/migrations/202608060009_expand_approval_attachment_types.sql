...
-- Remove rigid mime type restrictions in register_approval_attachment RPC
...
begin
  ...
  if not found or d.drafter_user_id <> auth.uid() or d.status <> 'draft'
     ...
     or p_file_size not between 1 and 20971520 then
  ...
  safe_mime := coalesce(nullif(btrim(p_mime_type), ''), 'application/octet-stream');
  ...
  insert into public.approval_attachments (
    ...
    d.id, d.current_revision_id, p_storage_path, left(btrim(p_original_name), 240), safe_mime, p_file_size, auth.uid()
