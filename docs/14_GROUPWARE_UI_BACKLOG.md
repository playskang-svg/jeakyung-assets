...
## 14.13 버튼 박스: 썸네일 카드 + 대상 팝업 — 완료

요청: "본문에는 썸네일과 메타설명글만 나오게 하고 클릭하면 선택한 링크페이지
팝업되고, 게시물을 클릭하면 같은 화면에서 열리고 목록 이동·닫기 등 기능을 갖춰줘."
(앞서 정한 "모두 새 탭"과 충돌해 확인한 뒤 **팝업 방식**으로 결정)

### 카드

`cards` 스타일은 이제 **썸네일 + 제목 + 설명** 구성이다(번호 배지와 "바로가기"
알약 버튼은 뺐다). 썸네일이 없으면 제목 앞 두 글자를 넣은 기본 배경이 나온다.
`tiles`·`list` 스타일은 그대로 두었다.

관리자 화면의 버튼 줄 맨 앞에 썸네일 칸이 생겼다. 눌러서 이미지를 올리면
공개 버킷 `public-site-media/buttons/` 에 저장되고, 올린 뒤에는 "썸네일 지우기"로
뺄 수 있다. (소식/정보 썸네일과 같은 버킷·같은 제한: 5MB, JPG·PNG·WebP·GIF)

### 대상 팝업

버튼을 누르면 `ButtonBoxTargetDialog` 가 뜬다.

- **게시판 대상**: 팝업 안에 게시판 목록이 뜨고, **글을 누르면 같은 팝업 안에서
  본문으로 바뀐다.** 되돌아오는 길은 `뒤로가기` / `목록 보기`(머리글·본문 하단)
  / `Esc`. 팝업을 닫는 것은 `×` / 바깥 클릭 / 본문이 아닐 때의 `Esc`.
  머리글의 `새 탭 ↗` 으로 전체 화면에서 열 수도 있다.
- **외부 주소**: 남의 사이트는 X-Frame-Options 때문에 팝업(iframe)에 담을 수 없다.
  그래서 새 탭으로 연다.

이를 위해 두 화면이 팝업 안에서도 동작하도록 바뀌었다.

| 파일 | 추가된 것 |
| --- | --- |
| `BoardPage.jsx` | `onOpenPost` — 있으면 글 클릭 시 라우팅 대신 콜백을 부른다 |
| `PostDetailPage.jsx` | `boardSlug`/`postId` props, `embedded`, `onBack` — 라우트 없이도 렌더되고 "목록 보기"가 콜백을 탄다 |

둘 다 props 가 없으면 예전과 똑같이 동작하므로 기존 라우트 화면은 영향이 없다.

### DB (`202608310001_button_box_thumbnails.sql`, 적용 완료)

- `button_box_items.thumbnail_url` 추가(http(s) 또는 `/` 로 시작하는 값만 허용).
- 조회 함수들이 `board_slug` / `page_slug` 를 함께 준다 — 팝업이 게시판을 바로
  열려면 slug 가 필요하다.

### 검증

빌드 후 청크 구성이 크게 바뀌어(`groupware-*.js` 142KB → 104KB, `supabase-*.js`
41KB 분리) **그룹웨어가 실제로 뜨는지 Chromium 으로 확인했다** — 로그인 화면이
정상 렌더되고 치명적 JS 오류 0건. Supabase 청크(207208바이트)와 URL·키도 그대로다.
